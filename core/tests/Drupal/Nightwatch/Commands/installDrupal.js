...
exports.command = function installDrupal(setupClass = '', callback) {
  ...
  const dbOption = process.env.DRUPAL_NIGHTWATCH_DB_URL.length > 0 ? `--db_url ${process.env.DRUPAL_NIGHTWATCH_DB_URL}` : '';
  let dbPrefix = '';
  ...
  try {
    ...
    const install = execSync(commandAsWebserver(`php ./scripts/test-site.php install --setup_class ${setupClass.replace(/\\/g, '\\\\')} --base_url ${process.env.DRUPAL_NIGHTWATCH_BASE_URL} ${dbOption} --json`));
    const installData = JSON.parse(install.toString());
    dbPrefix = installData.db_prefix;
    const matches = process.env.DRUPAL_NIGHTWATCH_BASE_URL.match(/^https?\:\/\/([^\/?#]+)(?:[\/?#]|$)/i);
    const domain = matches[1];
    const path = matches[2];
    this
      .url(process.env.DRUPAL_NIGHTWATCH_BASE_URL)
      .setCookie({
